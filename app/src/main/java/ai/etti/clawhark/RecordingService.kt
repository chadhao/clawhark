...
class RecordingService : Service() {
    ...
    override fun onCreate() {
        ...
        AppLog.i(TAG, "Codec: Opus ${config.opusBitRate/1000}kbps | Chunk: ${config.chunkDurationMs/60000}min | Upload: every ${config.uploadIntervalMinutes}min")
        
        if (config.isDebugMode) {
            CodecDetector.detectAndLog(config.opusBitRate)
        }
